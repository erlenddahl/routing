...
namespace RoutingApi.Geometry
{
    public class RequestCoordinate
    {
        ...
        public PointWgs84 GetLatLng()
        {
            if (X.HasValue && Y.HasValue) return new PointUtm33(X.Value, Y.Value, 0).ToWgs84();
            if (Lat.HasValue && Lng.HasValue) return new PointWgs84(Lat.Value, Lng.Value);
            throw new Exception("A coordinate must be given with either X and Y (UTM33) or Lat and Lng (WGS84).");
        }
    }
}